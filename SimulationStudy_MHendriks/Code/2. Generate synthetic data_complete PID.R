...
# We generate the synthetic data using two different imputation methods: predictive mean matching (PMM) 
# for the continuous variables and logistic regression for the binary outcome, and 
# (2) classification and regression trees (CART) for each variable in the data. Additionally, we 
# test two parameter specifications for CART: cp fixed at 1e-4 and 1e-32 for minbucket set to 3.
plan(multisession)
...
SynData_complete <- list(syn_pmm = syn_pmm, syn_cart_m = syn_cart_m, syn_cart_cpm = syn_cart_cpm)

save(SynData_complete, file = "Workspaces/1.2 Generate synthetic data_complete PID.Rdata")
#OR scenario 2
#save(SynData_complete, file = "Workspaces/2.2 Generate synthetic data_complete PID.Rdata")
